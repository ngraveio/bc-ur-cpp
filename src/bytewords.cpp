...
uint8_t decode_word(const string& word, size_t word_len) {
    ...
    ssize_t x = static_cast<ssize_t>(tolower(word[0]) - 'a');
    ssize_t y = static_cast<ssize_t>(tolower(word[word_len == 4 ? 3 : 1]) - 'a');
    if(!(0 <= x && x < static_cast<ssize_t>(dim) && 0 <= y && y < static_cast<ssize_t>(dim))) {
        throw runtime_error("Invalid Bytewords.");
    }
    size_t offset = y * dim + x;
...
string Bytewords::encode(style style, const ByteVector& bytes) {
    ...
    throw runtime_error("Unknown style");
}
